...
namespace Solution {
    ...
    inline info add(info f, int x) {
        info g = f;
        per (i, m - x, 0) (g[i + x] += f[i]) %= mod;
        g[0] = 1;
        return g;
    }

    inline info del(info f, int x) {
        info g = f;
        rep (i, 0, m - x) (((g[i + x] -= f[i]) %= mod) += mod) %= mod;
        g[0] = 1;
        return g;
    }
    ...
    void calc(int l, int r, const std::vector<Query> &qs) {
        ...
        init(f[mid + 1]);
        per (i, mid, l) f[i] = add(f[i + 1], w[i]);

        init(g[mid]);
        rep (i, mid + 1, r) g[i] = add(g[i - 1], w[i]);

        for (auto q : mqs) {
            ans[q.i] = 0;
            rep (k, 0, q.x) {
                (ans[q.i] += 1ll * f[q.l][k] * g[q.r][q.x - k] % mod) %= mod;
            }
        }
