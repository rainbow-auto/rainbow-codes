...
namespace Sol {
    struct SegTree{
        ...
        int qry(int now, int l, int r, int R) {
            if (not now) return 0x3f3f3f3f;
            if (r <= R) return tr[now].mn;
            int mid = (l + r) >> 1;
            if (R <= mid) return qry(tr[now].ls, l, mid, R);
            return std::min(tr[tr[now].ls].mn, qry(tr[now].rs, mid + 1, r, R));
        }
    } tr;
    ...
    inline void solve() {
        ...
        int R = n;
        per (L, n, 1) {
            int t = tr.qry(tr.rt, -1e9, 1e9, l[L] - 1);
            tr.mdf(tr.rt, -1e9, 1e9, r[L], L);
