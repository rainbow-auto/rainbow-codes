    ...
    inline void solve() {
        ...
        rep (t, 0, p + m + 1) {
            ...
            rep (k, 0, t - 1) {
                ...
                (ans += c * pw2[k] % mod * k % mod * choose(p, k) % mod * choose(m, t - 1 - k) % mod) %= mod;
            }
        }
        std::cout << ans << "\n";
    }
...
namespace Task2 {
    inline void solve() {
        ...
        rep (t, 0, p + m + 1) {
            int k = t - 1;
            (ans += ichoose(n, t) * pw2[k] % mod * k % mod * choose(p, k) % mod * choose(m, t - 1 - k) % mod) %= mod;
        }
        std::cout << ans << "\n";
    }
}
...
namespace Sol {
    inline void solve() {
        int n = 2 * p + m;
        i64 ans = (n + 1) * ksm(4ll, p) % mod * fac[p] % mod * fac[p] % mod * ifac[2 * p + 1] % mod;
        std::cout << ans << "\n";
    }
}
