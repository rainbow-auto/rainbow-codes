...
int main () {
	...
	for (int w = 1; ; w <<= 1, m = p) {
		p = 0;
		for (int i = len; i + w > len; i--) { id[++p] = i; }
		for (int i = 1; i <= len; i++) { if (sa[i] - w > 0) { id[++p] = sa[i] - w; } }

		memset (cnt, 0, sizeof (cnt));
		for (int i = 1; i <= len; i++) { cnt[key1[i] = rk[id[i]]] ++; }
		for (int i = 1; i <= m; i++) { cnt[i] += cnt[i - 1]; }
		for (int i = len; i >= 1; i--) { sa[cnt[key1[i]]--] = id[i]; }

		p = 0;
		memcpy (old_rk + 1, rk + 1, sizeof (int) * len);
		for (int i = 1; i <= len; i++) { rk[sa[i]] = comp (sa[i], sa[i - 1], w) ? p : ++p; }
