...
int a_id[maxn], b_id[maxn];
int S, T;

inline void buildGraph (int flag) {
	tot = 0;

	for (int i = 1; i <= m; i++) { a_id[i] = newNode (); }
	for (int i = 1; i <= n; i++) { b_id[i] = newNode (); }
	S = newNode (), T = newNode ();	

	for (int i = 1; i <= m; i++) { Net::addEdge (S, a_id[i], a[i], 0); }
	for (int i = 1; i <= n; i++) { Net::addEdge (b_id[i], T, b[i], 0); }

	for (int i = 1; i <= m; i++) {
		for (int j = 1; j <= n; j++) {
			Net::addEdge (a_id[i], b_id[j], 0x3f3f3f3f, c[i][j] * flag);
		}
	}
}
