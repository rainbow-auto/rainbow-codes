...
namespace Task1 {
	inline void solve() {
		int ans = 0;
		rep (i, 1, n) for (auto t : b[i]) ans = std::max(ans, t.second);
		rep (i, 1, n) for (auto t : c[i]) ans = std::max(ans, t.second);
...
void solve() {
	...
	if (m == 1) return Task1::solve(), void(0);
	if (m == 2) return Task2::solve(), void(0);
}
